...
class _BarberScheduleScreenState extends ConsumerState<BarberScheduleScreen>
  ...
  // DateTime.weekday: Monday=1 ... Sunday=7. Line up with the backend's
  // per-day workingHours map keys so weekday-1 indexes straight in.
  static const _dayKeys = [
    'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday'
  ];

  /// (barberId::YYYY-MM-DD) pairs we've already auto-generated slots for
  /// in this session — stops the ref.listen loop from re-firing on
  /// every rebuild after a manual delete. If a barber deletes all slots
  /// on an auto-generated day they get an empty EmptyState (with the
  /// manual add button), not a re-generation.
  final Set<String> _autoGeneratedKeys = <String>{};
  bool _autoGenBusy = false;
  ...
  /// Auto-generates slots the first time a barber lands on a date that
  /// has no schedule yet. Reads the persisted workingHours + slotDuration
  /// (Ish soatlari sahifasidan) and PUTs the computed HH:MM list to
  /// /schedule via the existing saveDaySchedule endpoint. Skipped when:
  ///   - we've already generated for this (barber, date) in this session
  ///   - the barber's profile hasn't loaded yet
  ///   - the day is marked closed in workingHours (dam olish kuni)
  ///   - the computed slot list ends up empty (invalid range)
  /// Failures are silent — the manual EmptyState "Jadval qo'shish"
  /// tugmasi is always still there as the escape hatch.
  Future<void> _maybeAutoGenerateForDate(
      String barberId, String date, DateTime d) async {
    final key = '$barberId::$date';
    if (_autoGeneratedKeys.contains(key)) return;
    if (_autoGenBusy) return;

    // Skip when the barber has ALREADY touched this day — even if the
    // resulting slot list is empty (i.e. 'Kunni yopish'). Without this
    // guard, re-entering the app after closing today would silently
    // re-generate a fresh schedule and undo the close.
    final savedSet = await ref
        .read(barberSavedDatesProvider(barberId).future)
        .catchError((_) => <String>{});
    if (savedSet.contains(date)) {
      _autoGeneratedKeys.add(key);
      return;
    }

    final profileAsync = ref.read(barberProfileProvider(barberId));
    ...
    final dayCfg = workingHours[_dayKeys[d.weekday - 1]];
    if (dayCfg is! Map) return;
    final isOpen = dayCfg['isOpen'] == true || dayCfg['enabled'] == true;
    if (!isOpen) return;

    final open =
        (dayCfg['open'] ?? dayCfg['start'] ?? '09:00').toString();
    final close =
        (dayCfg['close'] ?? dayCfg['end'] ?? '20:00').toString();
    final lunchStart = dayCfg['lunchStart']?.toString();
    final lunchEnd = dayCfg['lunchEnd']?.toString();
    final slotDuration = (profile['slotDuration'] as num?)?.toInt() ?? 30;
    ...
    final slots = _computeSlots(
      ...
      lunchEndHHMM: lunchEnd,
    );
    if (slots.isEmpty) return;

    _autoGeneratedKeys.add(key);
    if (mounted) setState(() => _autoGenBusy = true);
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(tr(
          ref,
          'mobile.barber.schedule.autoGenerated',
          'Bugungi jadval avtomatik yaratildi',
        )),
        duration: const Duration(seconds: 2),
      ));
    } catch (_) {
      // Silent: EmptyState + manual "Jadval qo'shish" is still there.
    } finally {
      if (mounted) setState(() => _autoGenBusy = false);
    }
  }

  /// Turns 'HH:mm' open/close + step minutes into a sorted list of
  /// slot start times. Skips any step that falls inside the optional
  /// lunch window so a barber's break in Ish soatlari carries through
  /// to auto-generated schedules for free.
  List<String> _computeSlots({
    required String openHHMM,
    required String closeHHMM,
    required int stepMinutes,
    String? lunchStartHHMM,
    String? lunchEndHHMM,
  }) {
    ...
    final oMin = mins(openHHMM);
    final cMin = mins(closeHHMM);
    if (cMin <= oMin || stepMinutes <= 0) return const [];

    final int? lStart = (lunchStartHHMM != null && lunchStartHHMM.isNotEmpty)
        ? mins(lunchStartHHMM)
        : null;
    final int? lEnd = (lunchEndHHMM != null && lunchEndHHMM.isNotEmpty)
        ? mins(lunchEndHHMM)
        : null;

    final out = <String>[];
    for (var m = oMin; m < cMin; m += stepMinutes) {
      if (lStart != null && lEnd != null && m >= lStart && m < lEnd) {
        continue;
      }
      final h = m ~/ 60;
      final min = m % 60;
  ...
  void _refreshDay(String barberId) {
    ...
    ref.invalidate(barberScheduledDatesProvider(barberId));
    // Saved-dates set also changes on close-day (new empty row) and on
    // any first-time save — invalidate it too so the next screen visit
    // sees the fresh set and doesn't re-auto-generate.
    ref.invalidate(barberSavedDatesProvider(barberId));
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          slotsAsync.when(
            ...
            data: (slots) {
              if (slots.isEmpty) {
                // Kick off auto-generation on the next frame so we
                // don't call setState / mutate providers during build.
                // _maybeAutoGenerateForDate no-ops after the first
                // attempt per (barberId, date) so this can't loop even
                // though the callback re-runs on every rebuild.
                final dateStr = _dateStr(_selectedDate);
                final selectedDay = _selectedDate;
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  _maybeAutoGenerateForDate(barberId, dateStr, selectedDay);
                });
                if (_autoGenBusy) {
